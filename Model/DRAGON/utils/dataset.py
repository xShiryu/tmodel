...
class RecDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        pos_item = self.df.iloc[idx, 1]

        filtered_rows = self.df[self.df.iloc[:, 0] == user]
        interacted_item = filtered_rows.iloc[:, 1].tolist()
        all_item = pd.unique(self.df.iloc[:, 1]).tolist()

        neg_item = random.sample(set(all_item)-set(interacted_item), 5)
        
        user_tensor = torch.LongTensor([user]*6)
        item_tensor = torch.LongTensor([pos_item]+neg_item)"""
        # Series result
        return self.df.iloc[idx]#, user_tensor, item_tensor
